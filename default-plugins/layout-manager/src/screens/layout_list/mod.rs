...
use crate::ui::{Controls, LayoutsTable, LayoutDetail};
use crate::text_input::InputAction;
use super::{Screen, KeyResponse, OptimisticUpdate};
...
impl LayoutListScreen {
    ...
    pub fn handle_key(&mut self, key: KeyWithModifier, display_layouts: &[DisplayLayout]) -> KeyResponse {
    ...
    fn handle_typing_filter_mode(&mut self, key: KeyWithModifier, display_layouts: &[DisplayLayout]) -> KeyResponse {
        ...
        match key.bare_key {
            ...
            BareKey::Enter if key.has_no_modifiers() => {
                if self.search_state.get_filter_input().is_empty() || self.search_state.get_search_results().is_empty() {
                    self.clear_filter();
                } else {
    ...
    fn start_new_layout_creation_from_session(&self) -> Screen {
        match dump_session_layout() {
            ...
            Ok((_, None)) => {
                Screen::Error(super::ErrorScreen {
    ...
    fn navigate_up(&mut self, display_layouts: &[DisplayLayout]) {
        if self.is_searching() {
            ...
            if self.search_state.get_selected_search_index() == 0 {
                self.search_state.set_selected_search_index(self.search_state.get_search_results().len().saturating_sub(1));
            } else {
                let new_index = self.search_state.get_selected_search_index().saturating_sub(1);
                self.search_state.set_selected_search_index(new_index);
            }
            // Keep selected_layout_index synchronized with the actual position
            self.selected_layout_index = self.search_state.get_search_results()
    ...
    fn navigate_down(&mut self, display_layouts: &[DisplayLayout]) {
        if self.is_searching() {
            // Navigate in search results
            if self.search_state.get_selected_search_index() + 1 >= self.search_state.get_search_results().len() {
    ...
    fn apply_selected_layout(&self, display_layouts: &[DisplayLayout]) {
        if let Some(DisplayLayout::Valid(chosen_layout)) = display_layouts.get(self.selected_layout_index) {
            override_layout(
                chosen_layout,
    ...
    fn show_error_detail(&self, display_layouts: &[DisplayLayout]) -> Option<Screen> {
        if let Some(DisplayLayout::Error { name, error, error_message: _ }) =
    ...
    pub fn render(&mut self, display_layouts: &[DisplayLayout], rows: usize, cols: usize) {
        ...
        let (total_width, total_height) = self.calculate_content_dimensions(rows, cols, display_layouts);
        let (base_x, base_y) = self.calculate_base_coordinates(rows, cols, total_width, total_height);
        ...
        let (table_width, detail_x) = self.calculate_horizontal_split(cols, display_layouts, controls_width);

        let table_y = if self.search_state.is_active() || self.search_state.is_typing() { 1 + base_y } else { base_y };
        ...
        let (visible_layouts, selected_in_window, hidden_above, hidden_below) = calculate_visible_window(
            &layouts_to_render,
            selected_index,
            content_height.saturating_sub(1), // account for title row
        );

        let matched_indices = self.search_state.get_matched_indices_for_visible(hidden_above, visible_layouts.len());

        let has_visible_layouts = !visible_layouts.is_empty();
        LayoutsTable::new(visible_layouts, selected_in_window, hidden_above, hidden_below)
            .with_matched_indices(matched_indices)
            .render(base_x, table_y, content_height.saturating_sub(1), table_width);

        if has_visible_layouts {
            if let Some(selected_layout) = display_layouts.get(self.selected_layout_index) {
                LayoutDetail::new(selected_layout)
                    .render(detail_x + base_x, table_y + 1, content_height.saturating_sub(1), table_width);
            } else {
    ...
    fn calculate_layout(&self, rows: usize, display_layouts: &[DisplayLayout]) -> (usize, usize) {
        ...
        if content_height + controls_height + padding + filter_row_height >= rows {
            content_height = rows.saturating_sub(controls_height).saturating_sub(padding).saturating_sub(filter_row_height)
        }
    ...
    fn get_controls_height(&self) -> usize {
        if self.show_more_override_options { 5 } else { 3 }
    }
    ...
    fn calculate_content_dimensions(&self, rows: usize, cols: usize, display_layouts: &[DisplayLayout]) -> (usize, usize) {
        let filter_row_height = if self.is_searching() { 1 } else { 0 };
    ...
    fn calculate_base_coordinates(&self, rows: usize, cols: usize, total_width: usize, total_height: usize) -> (usize, usize) {
    ...
    fn is_searching(&self) -> bool {
        (self.search_state.is_active() && !self.search_state.get_search_results().is_empty()) || self.search_state.is_typing()
    }
    ...
    fn effective_layouts(&self, display_layouts: &[DisplayLayout]) -> Vec<DisplayLayout> {
        if self.is_searching() {
            self.search_state.get_search_results().iter().map(|r| r.layout.clone()).collect()
        } else {
    ...
    fn update_filter(&mut self, display_layouts: &[DisplayLayout]) {
        let (total_width, total_height) = self.calculate_content_dimensions(self.last_rows, self.last_cols, display_layouts);
        let (base_x, base_y) = self.calculate_base_coordinates(self.last_rows, self.last_cols, total_width, total_height);

        self.search_state.update_filter(display_layouts, base_x, base_y);
...
fn calculate_visible_window(
    ...
    // returns: (index_in_rendered, hidden_items_above, hidden_items_below)
      ...
      (visible, adjusted_index, before, layouts.len().saturating_sub(after + 1))
  }
