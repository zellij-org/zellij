...
impl App {
    ...
    pub fn update_highlighted_panes(&self) {
        ...
        if let Some(marked_index) = &self.marked_index {
            if self.visibility_and_focus.left_side_is_focused() {
                if let Some(main_index_pane_id) = self.search_results
                    .as_ref()
                ...
                if let Some(main_index_pane_id) = self.right_side_panes.get(marked_index.main_index).map(|p| p.id) {
                    pane_ids_to_highlight.push(main_index_pane_id);
                }
                for index in &marked_index.additional_indices {
    ...
    pub fn group_panes(&mut self, mut marked_index: MarkedIndex, keep_left_side_focused: bool) {
        let mut all_selected_indices: BTreeSet<usize> = marked_index.additional_indices.drain().collect();
    ...
    pub fn ungroup_panes(&mut self, mut marked_index: MarkedIndex) {
        ...
        if self.right_side_panes.is_empty() {
            ...
            self.marked_index = Some(MarkedIndex::new(self.right_side_panes.len().saturating_sub(1)));
            self.visibility_and_focus.show_both_sides();
        } else {
            self.marked_index = Some(marked_index);
            self.visibility_and_focus.show_both_sides();
    ...
    pub fn move_marked_index_down(&mut self) {
        match self.marked_index.as_mut() {
            Some(marked_index) =>{
                ...
                if self.visibility_and_focus.left_side_is_focused() && is_searching && marked_index.main_index == search_result_count.saturating_sub(1) {
                    marked_index.main_index = 0;
                } else if self.visibility_and_focus.left_side_is_focused() && !is_searching && marked_index.main_index == self.left_side_panes.len().saturating_sub(1) {
                    marked_index.main_index = 0;
                } else if self.visibility_and_focus.right_side_is_focused() && marked_index.main_index == self.right_side_panes.len().saturating_sub(1) {
                    marked_index.main_index = 0;
                } else {
    ...
    pub fn move_marked_index_up(&mut self) {
        match self.marked_index.as_mut() {
            Some(marked_index) =>{
                ...
                } else if self.visibility_and_focus.right_side_is_focused() && marked_index.main_index == 0 {
            ...
            None => {
                if self.visibility_and_focus.left_side_is_focused() {
                    ...
                    if is_searching && has_search_results {
                        ...
                        self.marked_index = Some(MarkedIndex::new(search_results_count.saturating_sub(1)));
                    } else if !is_searching && !self.left_side_panes.is_empty() {
                        self.marked_index = Some(MarkedIndex::new(self.left_side_panes.len().saturating_sub(1)));
                    }
                } else if self.visibility_and_focus.right_side_is_focused() && !self.right_side_panes.is_empty() {
                    self.marked_index = Some(MarkedIndex::new(self.right_side_panes.len().saturating_sub(1)));
                }
