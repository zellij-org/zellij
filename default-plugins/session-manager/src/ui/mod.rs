...
use crate::session_list::{SessionList, SelectedIndex};
use components::{LineToRender, minimize_lines, build_session_ui_line, build_tab_ui_line, build_pane_ui_line, Colors};
...
macro_rules! render_assets {
    ($assets:expr, $line_count_to_remove:expr, $selected_index:expr, $to_render_until_selected: expr, $to_render_after_selected:expr, $has_deeper_selected_assets:expr, $max_cols:expr, $colors:expr) => {{
        ...
        let (start_index, anchor_asset_index, end_index, line_count_to_remove) = minimize_lines($assets.len(), $line_count_to_remove, $selected_index);
        ...
        if let Some(assets_to_render_before_selected) = $assets.get(start_index..anchor_asset_index) {
            for asset in assets_to_render_before_selected {
                let mut asset: LineToRender = asset.as_line_to_render(current_index, $max_cols, $colors);
        ...
        if let Some(selected_asset) = $assets.get(anchor_asset_index) {
            if $selected_index.is_some() && !$has_deeper_selected_assets {
                let mut selected_asset: LineToRender = selected_asset.as_line_to_render(current_index, $max_cols, $colors);
                ...
                $to_render_until_selected.push(selected_asset.as_line_to_render(current_index, $max_cols, $colors));
                current_index += 1;
            }
        }
        if let Some(assets_to_render_after_selected) = $assets.get(anchor_asset_index + 1..end_index) {
...
impl SessionList {
    ...
    fn render_list(&self, max_rows: usize, max_cols: usize, colors: Colors) -> Vec<LineToRender> {
        ...
        let line_count_to_remove = self.render_sessions(&mut lines_to_render_until_selected, &mut lines_to_render_after_selected, line_count_to_remove, max_cols, colors);
        let line_count_to_remove = self.render_tabs(&mut lines_to_render_until_selected, &mut lines_to_render_after_selected, line_count_to_remove, max_cols, colors);
        self.render_panes(&mut lines_to_render_until_selected, &mut lines_to_render_after_selected, line_count_to_remove, max_cols, colors);
        let mut lines_to_render = lines_to_render_until_selected;
        lines_to_render.append(&mut lines_to_render_after_selected);
        lines_to_render
    }
    fn render_sessions(&self, to_render_until_selected: &mut Vec<LineToRender>, to_render_after_selected: &mut Vec<LineToRender>, line_count_to_remove: usize, max_cols: usize, colors: Colors) -> usize{
        render_assets!(self.session_ui_infos, line_count_to_remove, self.selected_index.0, to_render_until_selected, to_render_after_selected, self.selected_index.1.is_some(), max_cols, colors)
    }
    fn render_tabs(&self, to_render_until_selected: &mut Vec<LineToRender>, to_render_after_selected: &mut Vec<LineToRender>, line_count_to_remove: usize, max_cols: usize, colors: Colors) -> usize {
        if self.selected_index.1.is_none() {
            return line_count_to_remove;
        }
        if let Some(tabs_in_session) = self.selected_index.0.and_then(|i| self.session_ui_infos.get(i)).map(|s| &s.tabs) {
            render_assets!(tabs_in_session, line_count_to_remove, self.selected_index.1, to_render_until_selected, to_render_after_selected, self.selected_index.2.is_some(), max_cols, colors)
        } else {
            line_count_to_remove
        }
    }
    fn render_panes(&self, to_render_until_selected: &mut Vec<LineToRender>, to_render_after_selected: &mut Vec<LineToRender>, line_count_to_remove: usize, max_cols: usize, colors: Colors) -> usize {
        if self.selected_index.2.is_none() {
            return line_count_to_remove;
        }
        if let Some(panes_in_session) = self.selected_index.0.and_then(|i| self.session_ui_infos.get(i)).map(|s| &s.tabs).and_then(|tabs| {
            self.selected_index.1.and_then(|i| tabs.get(i)).map(|t| &t.panes)
        }) {
            render_assets!(panes_in_session, line_count_to_remove, self.selected_index.2, to_render_until_selected, to_render_after_selected, false, max_cols, colors)
        } else {
            line_count_to_remove
        }
    }
    fn total_lines_to_render(&self) -> usize {
        self.session_ui_infos.iter().enumerate().fold(0, |acc, (index, s)| {
...
impl SessionUiInfo {
    pub fn from_session_info(session_info: &SessionInfo) -> Self {
        SessionUiInfo {
            ...
            tabs: session_info.tabs.iter().map(|t| TabUiInfo::new(t, &session_info.panes)).collect(),
    ...
    pub fn line_count(&self, selected_index: &SelectedIndex) -> usize {
        ...
        if selected_index.tabs_are_visible() {
            match selected_index.selected_tab_index()
    ...
    fn as_line_to_render(&self, _session_index: u8, mut max_cols: usize, colors: Colors) -> LineToRender {
...
impl TabUiInfo {
    pub fn new(tab_info: &TabInfo, pane_manifest: &PaneManifest) -> Self {
        let panes = pane_manifest.panes.get(&tab_info.position).map(|p| p.iter().filter_map(|pane_info| {
