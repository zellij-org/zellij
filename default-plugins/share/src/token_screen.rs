...
const EXPLANATION_2_LONG: &str = "Copy this token, because it will not be saved and can't be retrieved.";
...
impl TokenScreen {
    ...
    fn prepare_screen_elements(&self) -> ScreenElements {
        ...
        let explanation_variants = [
            TextVariant { long: EXPLANATION_1_LONG, short: EXPLANATION_1_SHORT },
            TextVariant { long: EXPLANATION_2_LONG, short: EXPLANATION_2_SHORT },
            TextVariant { long: EXPLANATION_3_LONG, short: EXPLANATION_3_SHORT },
        ];

        let token_label = token_variant.select(self.cols.saturating_sub(self.token.chars().count().saturating_sub(1)));
    ...
    fn calculate_max_width(&self, elements: &ScreenElements) -> usize {
        ...
        let explanation_widths = elements.explanation_texts.iter()
    ...
    fn render_elements(&self, elements: &ScreenElements, base_x: usize, base_y: usize) {
        print_text_with_coordinates(elements.token.clone(), base_x, base_y + TOKEN_Y_OFFSET, None, None);
        
        let y_offsets = [EXPLANATION_1_Y_OFFSET, EXPLANATION_2_Y_OFFSET, EXPLANATION_3_Y_OFFSET];
        ...
        print_text_with_coordinates(elements.esc.clone(), base_x, base_y + ESC_Y_OFFSET, None, None);
    }
