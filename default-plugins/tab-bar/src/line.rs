...
fn left_more_message(tab_count_to_the_left: usize, palette: Palette, separator: &str) -> LinePart {
    ...
    let left_separator = style!(palette.black, palette.orange).paint(separator);
    let more_styled_text = style!(palette.black, palette.orange)
        .bold()
        .paint(more_text);
    let right_separator = style!(palette.orange, palette.black).paint(separator);
...
fn tab_line_prefix(session_name: Option<&str>, palette: Palette, cols: usize) -> Vec<LinePart> {
    ...
    let prefix_styled_text = style!(palette.white, palette.black)
        .bold()
        .paint(prefix_text);
    ...
    if let Some(name) = session_name {
        ...
        let name_part_styled_text = style!(palette.white, palette.black).bold().paint(name_part);
