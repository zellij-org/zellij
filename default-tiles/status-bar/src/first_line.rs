...
use crate::colors::{BLACK, WHITE, GRAY, GREEN, BRIGHT_GRAY, RED};
...
fn unselected_mode_shortcut(letter: char, text: &str) -> LinePart {
    ...
    let char_left_separator = Style::new().bold().fg(BLACK).on(BRIGHT_GRAY).bold().paint(format!(" <"));
    let char_shortcut = Style::new().bold().fg(RED).on(BRIGHT_GRAY).bold().paint(format!("{}", letter));
    let char_right_separator = Style::new().bold().fg(BLACK).on(BRIGHT_GRAY).bold().paint(format!(">"));
    let styled_text = Style::new().fg(BLACK).on(BRIGHT_GRAY).bold().paint(format!("{} ", text));
...
fn selected_mode_shortcut(letter: char, text: &str) -> LinePart {
    ...
    let char_left_separator = Style::new().bold().fg(BLACK).on(GREEN).bold().paint(format!(" <"));
    let char_shortcut = Style::new().bold().fg(RED).on(GREEN).bold().paint(format!("{}", letter));
    let char_right_separator = Style::new().bold().fg(BLACK).on(GREEN).bold().paint(format!(">"));
    let styled_text = Style::new().fg(BLACK).on(GREEN).bold().paint(format!("{} ", text));
...
fn disabled_mode_shortcut(text: &str) -> LinePart {
    let prefix_separator = Style::new().fg(GRAY).on(BRIGHT_GRAY).paint(ARROW_SEPARATOR);
    let styled_text = Style::new().fg(GRAY).on(BRIGHT_GRAY).dimmed().paint(format!("{} ", text));
    let suffix_separator = Style::new().fg(BRIGHT_GRAY).on(GRAY).paint(ARROW_SEPARATOR);
    LinePart {
...
fn selected_mode_shortcut_single_letter(letter: char) -> LinePart {
    ...
    let prefix_separator = Style::new().fg(GRAY).on(GREEN).paint(ARROW_SEPARATOR);
    let char_shortcut = Style::new().bold().fg(RED).on(GREEN).bold().paint(char_shortcut_text);
    let suffix_separator = Style::new().fg(GREEN).on(GRAY).paint(ARROW_SEPARATOR);
    LinePart {
        part: format!("{}", ANSIStrings(&[prefix_separator, char_shortcut, suffix_separator])),
        len,
    }
}
...
fn unselected_mode_shortcut_single_letter(letter: char) -> LinePart {
    ...
    let len = char_shortcut_text.chars().count() + 4; // 2 for the arrows, 2 for the padding
    let prefix_separator = Style::new().fg(GRAY).on(BRIGHT_GRAY).paint(ARROW_SEPARATOR);
    let char_shortcut = Style::new().bold().fg(RED).on(BRIGHT_GRAY).bold().paint(char_shortcut_text);
    let suffix_separator = Style::new().fg(BRIGHT_GRAY).on(GRAY).paint(ARROW_SEPARATOR);
    LinePart {
        part: format!("{}", ANSIStrings(&[prefix_separator, char_shortcut, suffix_separator])),
        len,
...
fn single_letter_ctrl_key(key: &CtrlKeyShortcut) -> LinePart {
    ...
    match key.mode {
        CtrlKeyMode::Unselected => {
            unselected_mode_shortcut_single_letter(letter_shortcut)
        },
        CtrlKeyMode::Selected => {
            selected_mode_shortcut_single_letter(letter_shortcut)
        }
        CtrlKeyMode::Disabled => {
            disabled_mode_shortcut(&format!(" {}", letter_shortcut))
        }
    }
}
