...
impl<'a> PaneResizer <'a>{
    pub fn new(panes: &'a mut BTreeMap<PaneId, Box<dyn Pane>>, os_api: &'a mut Box<dyn OsApi>) -> Self {
        PaneResizer {
            panes,
            os_api,
        }
    }
    pub fn resize(&mut self, mut current_size: PositionAndSize, new_size: PositionAndSize) -> Option<(isize, isize)> { // (column_difference, row_difference)
    ...
    fn reduce_panes_left_and_pull_adjacents_left(&mut self, panes_to_reduce: Vec<PaneId>, reduce_by: usize) {
    ...
    fn reduce_panes_up_and_pull_adjacents_up(&mut self, panes_to_reduce: Vec<PaneId>, reduce_by: usize) {
        let mut pulled_panes: HashSet<PaneId> = HashSet::new();
        for pane_id in panes_to_reduce {
            let (pane_x, pane_y, pane_columns, pane_rows) = {
                let pane = self.panes.get(&pane_id).unwrap();
    ...
    fn increase_panes_down_and_push_down_adjacents(&mut self, panes_to_increase: Vec<PaneId>, increase_by: usize) {
    ...
    fn increase_panes_right_and_push_adjacents_right(&mut self, panes_to_increase: Vec<PaneId>, increase_by: usize) {
        let mut pushed_panes: HashSet<PaneId> = HashSet::new();
        for pane_id in panes_to_increase {
            let (pane_x, pane_y, pane_columns, pane_rows) = {
                let pane = self.panes.get(&pane_id).unwrap();
...
fn find_next_increasable_horizontal_pane(panes: &BTreeMap<PaneId, Box<dyn Pane>>, right_of: &Box<dyn Pane>, increase_by: usize) -> Option<PaneId> {
    let next_pane_candidates = panes.values().filter(|p|
        p.x() == right_of.x() + right_of.columns() + 1 &&
        p.horizontally_overlaps_with(right_of.as_ref()) // TODO: the name here is wrong, it should be vertically_overlaps_with
    );
    let resizable_candidates = next_pane_candidates.filter(|p| p.can_increase_height_by(increase_by));
    resizable_candidates.fold(None, |next_pane_id, p| {
        match next_pane_id {
            Some(next_pane) => {
...
fn find_next_increasable_vertical_pane(panes: &BTreeMap<PaneId, Box<dyn Pane>>, below: &Box<dyn Pane>, increase_by: usize) -> Option<PaneId> {
    let next_pane_candidates = panes.values().filter(|p|
        p.y() == below.y() + below.rows() + 1 &&
        p.vertically_overlaps_with(below.as_ref()) // TODO: the name here is wrong, it should be horizontally_overlaps_with
    );
    let resizable_candidates = next_pane_candidates.filter(|p| p.can_increase_width_by(increase_by));
    resizable_candidates.fold(None, |next_pane_id, p| {
        match next_pane_id {
            Some(next_pane) => {
                let next_pane = panes.get(&next_pane).unwrap();
...
fn find_next_reducible_vertical_pane(panes: &BTreeMap<PaneId, Box<dyn Pane>>, below: &Box<dyn Pane>, reduce_by: usize) -> Option<PaneId> {
    let next_pane_candidates = panes.values().filter(|p|
        p.y() == below.y() + below.rows() + 1 &&
        p.vertically_overlaps_with(below.as_ref()) // TODO: the name here is wrong, it should be horizontally_overlaps_with
    );
...
fn find_next_reducible_horizontal_pane(panes: &BTreeMap<PaneId, Box<dyn Pane>>, right_of: &Box<dyn Pane>, reduce_by: usize) -> Option<PaneId> {
    let next_pane_candidates = panes.values().filter(|p|
        p.x() == right_of.x() + right_of.columns() + 1 &&
        p.horizontally_overlaps_with(right_of.as_ref()) // TODO: the name here is wrong, it should be vertically_overlaps_with
    );
...
fn find_increasable_vertical_chain(
    ...
    loop {
        ...
        match panes.values().find(|p| p.y() == 0 && p.x() == vertical_coordinate) {
            Some(topmost_pane) => {
