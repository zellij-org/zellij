...
pub(crate) fn start_client(opts: CliArgs) {
    ...
    let (config, layout, config_options, mut config_without_layout, mut config_options_without_layout) =
    ...
    loop {
        ...
        if let Some(reconnect_to_session) = &reconnect_to_session {
            ...
            reload_config_from_disk(&mut config_without_layout, &mut config_options_without_layout, &opts);
...
fn reload_config_from_disk(config_without_layout: &mut Config, config_options_without_layout: &mut Options, opts: &CliArgs) {
    match Setup::from_cli_args(&opts) {
