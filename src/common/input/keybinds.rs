...
fn get_defaults_for_mode(mode: &InputMode) -> Result<ModeKeybinds, String> {
    ...
    match *mode {
        InputMode::Normal => {
            defaults.insert(Key::Ctrl('g'), vec![Action::SwitchToMode(InputMode::Command)]);
        }
        InputMode::Command => {
            defaults.insert(Key::Char('r'), vec![Action::SwitchToMode(InputMode::Resize)]);
        ...
        InputMode::Pane => {
            ...
            defaults.insert(Key::Char('r'), vec![Action::NewPane(Some(Direction::Right))]);
