...
pub use zellij_utils::distribution::{Distribution, DistributionLayout, DistributionPlugin};

/// Run Zellij as the given distribution.
///
/// A distribution bundles its own plugins, configuration and layouts into the executable. Its
/// plugins are referenced as `zellij:<name>`, are loaded from memory rather than from disk, and
/// are not subject to the plugin permission system, exactly as Zellij's own builtin plugins are.
///
/// Must be called at most once, before anything else in the process reads configuration.
pub fn run(distribution: Distribution) {
    if let Err(e) = set_distribution(distribution) {
