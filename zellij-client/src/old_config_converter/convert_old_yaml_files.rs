...
pub fn convert_old_yaml_files(opts: &CliArgs) {
    ...
    if let Some(layout_dir) = layout_dir {
        if let Ok(files) = std::fs::read_dir(layout_dir) {
            for file in files {
                if let Ok(file) = file {
                    if file.path().extension().map(|s| s.to_string_lossy().to_string()) == Some("yaml".into()) {
    ...
    if let Some(config_dir) = config_dir {
        let yaml_config_location = specified_config_location
            .cloned()
            .filter(|c| c.extension().map(|s| s.to_string_lossy().to_string()) == Some("yaml".into()));
        let specified_yaml_config_location = yaml_config_location.is_some();
        let config_location = yaml_config_location.unwrap_or_else(|| config_dir.join(OLD_CONFIG_NAME));
        match convert_yaml(config_location, layout_files_to_convert, theme_files_to_convert, specified_yaml_config_location) {
            Ok(should_exit) => {
                if should_exit {
                    std::process::exit(0);
                }
...
fn print_no_actions_and_wait_for_user_input() -> Result<(), String> {
    ...
    std::io::stdin().read_line(&mut String::new()).map_err(|e| format!("Failed to read from STDIN: {:?}", e))?;
    Ok(())
}
...
fn print_flag_help_message(layout_files_to_convert: Vec<(PathBuf, bool)>, yaml_config_file: &PathBuf, yaml_config_was_explicitly_set: bool) -> Result<(), String> {
    ...
    match layout_files_to_convert.iter().find(|(_f, explicit)| *explicit) {
        Some((explicitly_specified_layout, _)) => {
            ...
            if yaml_config_was_explicitly_set {
                ...
                println!(
                    ...
                    kdl_config_file_path.as_path().as_os_str().to_string_lossy().to_string(),
                    kdl_explicitly_specified_layout.as_path().as_os_str().to_string_lossy().to_string(),
                );
            } else {
...
fn convert_layouts(layout_files_to_convert: Vec<(PathBuf, bool)>) -> Result<(), String> {
    for (layout_file, _was_explicitly_set) in layout_files_to_convert {
        let raw_layout_file = std::fs::read_to_string(&layout_file).map_err(|e| format!("Failed to read layout file {:?}: {:?}", layout_file, e))?;
        ...
        std::fs::write(&new_layout_file, kdl_layout).map_err(|e| format!("Failed to write new layout file to {:?}: {:?}", new_layout_file, e))?;
    }
    Ok(())
}
...
fn convert_themes(theme_files_to_convert: Vec<PathBuf>) -> Result<(), String> {
    for theme_file in theme_files_to_convert {
        let raw_theme_file = std::fs::read_to_string(&theme_file).map_err(|e| format!("Failed to read theme file {:?}: {:?}", theme_file, e))?;
        ...
        std::fs::write(&new_theme_file, kdl_theme).map_err(|e| format!("Failed to write new theme file to {:?}: {:?}", new_theme_file, e))?;
    }
    Ok(())
}
...
fn convert_config(yaml_config_file: PathBuf, new_config_file: PathBuf) -> Result<(), String> {
    if yaml_config_file.exists() && !new_config_file.exists() {
        let raw_config_file = std::fs::read_to_string(&yaml_config_file).map_err(|e| format!("Failed to read config file {:?}: {:?}", yaml_config_file, e))?;
        let kdl_config = config_yaml_to_config_kdl(&raw_config_file, false)?;
        std::fs::write(&new_config_file, kdl_config).map_err(|e| format!("Failed to write new config file to {:?}: {:?}", new_config_file, e))?;
    }
    Ok(())
}

fn convert_yaml(yaml_config_file: PathBuf, layout_files_to_convert: Vec<(PathBuf, bool)>, theme_files_to_convert: Vec<PathBuf>, yaml_config_was_explicitly_set: bool) -> Result<bool, String> {
    ...
    let layout_was_explicitly_set = layout_files_to_convert.iter().find(|(_l, was_explicitly_set)| *was_explicitly_set).is_some();
    let new_config_file_exists = new_config_file.exists();
    let no_need_to_convert_config = (new_config_file_exists && !yaml_config_was_explicitly_set) || !yaml_config_file_exists;
    if no_need_to_convert_config && layout_files_to_convert.is_empty() && theme_files_to_convert.is_empty() && !layout_was_explicitly_set {
        // Nothing to do...
