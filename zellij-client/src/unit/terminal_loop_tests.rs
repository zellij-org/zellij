...
async fn test_stdin_forwarded_to_terminal_websocket() {
    ...
    let terminal_tcp = TcpStream::connect(format!("127.0.0.1:{}", port)).await.unwrap();
    let (terminal_ws, _) = tokio_tungstenite::client_async_with_config(&terminal_url, MaybeTls::Plain(terminal_tcp), None).await.unwrap();
    let control_tcp = TcpStream::connect(format!("127.0.0.1:{}", port)).await.unwrap();
    let (control_ws, _) = tokio_tungstenite::client_async_with_config(&control_url, MaybeTls::Plain(control_tcp), None).await.unwrap();
