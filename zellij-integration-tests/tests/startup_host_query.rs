...
fn attach_startup_replies_are_not_forwarded_into_pane_query() {
    ...
    let stdout_baseline = reattached
        .wait_for_stdout_bytes("capture stdout baseline before pane query", |_| true)
        .len();
    ...
    reattached.wait_for_stdout_bytes("pane host-query forward opened", |bytes| {
