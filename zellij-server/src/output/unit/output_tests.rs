...
fn test_character_chunk_drain_by_width() {
    ...
    let drained_text: String = drained.iter().map(|c| c.grapheme()).collect();
...
fn test_character_chunk_cut_middle_out() {
    ...
    let left_text: String = left.iter().map(|c| c.grapheme()).collect();
    assert_eq!(left_text, "Hello", "Left chunk should be 'Hello'");

    let right_text: String = right.iter().map(|c| c.grapheme()).collect();
