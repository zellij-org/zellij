...
impl PaneGroups {
    ...
    pub fn toggle_pane_id_in_group(&mut self, pane_id: PaneId, screen_size: Size, client_id: &ClientId) {
    ...
    pub fn add_pane_id_to_group(&mut self, pane_id: PaneId, screen_size: Size, client_id: &ClientId) {
        let previous_groups = self.clone_inner();
        let client_pane_group = self.panes_in_group.entry(*client_id).or_insert_with(|| vec![]);
        if !client_pane_group.contains(&pane_id) {
            client_pane_group.push(pane_id);
    ...
    fn should_launch_plugin(&self, previous_groups: &HashMap<ClientId, Vec<PaneId>>, client_id: &ClientId) -> bool {
