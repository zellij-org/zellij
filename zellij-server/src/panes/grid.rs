...
impl Click {
    pub fn record_click(&mut self, position: Position) {
        let click_is_same_position_as_last_click = self.position_and_time.map(|(p, _t)| p == position).unwrap_or(false);
        let click_is_within_time_threshold = self.position_and_time.map(|(_p, t)| t.elapsed().as_millis() <= CLICK_TIME_THRESHOLD).unwrap_or(false);
...
impl Grid {
    ...
    pub fn start_selection(&mut self, start: &Position) {
        ...
        if self.click.is_double_click() {
            ...
            self.selection.set_start_and_end_positions(start_position, end_position);
            for i in std::cmp::min(start_position.line.0, end_position.line.0)..=std::cmp::max(start_position.line.0, end_position.line.0) {
                self.output_buffer.update_line(i as usize);
            }
            self.mark_for_rerender();
            return;
        } else if self.click.is_triple_click() {
            let Some((start_position, end_position)) = self.canonical_line_around_position(&start) else {
                // no-op
                return;
            };
            self.selection.set_start_and_end_positions(start_position, end_position);
    ...
    pub fn word_around_position(&self, position: &Position) -> Option<(Position, Position)> {
        ...
        let (index_start, index_end) = position_row.word_indices_around_character_index(position.column.0)?;
        ...
        while !position_row_is_canonical && position_start.column.0 == 0 {
            if let Some(position_row_above) = self.viewport.get(position_start.line.0.saturating_sub(1) as usize) {
                let new_start_index = position_row_above.word_start_index_of_last_character();
                position_start = Position::new(position_start.line.0.saturating_sub(1) as i32, new_start_index as u16);
    ...
    pub fn canonical_line_around_position(&self, position: &Position) -> Option<(Position, Position)> {
        ...
        let mut position_end = Position::new(position.line.0 as i32, position_row.columns.len() as u16);

        let mut found_canonical_row_start = position_row.is_canonical;
        while !found_canonical_row_start {
            if let Some(row_above) = self.viewport.get(position_start.line.0.saturating_sub(1) as usize) {
        ...
        while !found_canonical_row_end {
            if let Some(row_below) = self.viewport.get(position_end.line.0 as usize + 1) {
                if row_below.is_canonical {
                    ...
                    position_end = Position::new(position_end.line.0 as i32 + 1, row_below.columns.len() as u16);
                }
            } else {
                break;
            }
...
impl Row {
    ...
    pub fn word_indices_around_character_index(&self, index: usize) -> Option<(usize, usize)> {
        ...
        let mut end_position = self.columns.iter().enumerate().skip(index).find_map(|(i, t_c)| if is_selection_boundary_character(t_c.character) { Some(i) } else { None }).unwrap_or_else(|| self.columns.len());
        let start_position = self.columns.iter().enumerate().take(index).rev().find_map(|(i, t_c)| if is_selection_boundary_character(t_c.character) { Some(i + 1) } else { None }).unwrap_or(0);
    ...
    pub fn word_start_index_of_last_character(&self) -> usize {
        self.columns.iter().enumerate().rev().find_map(|(i, t_c)| if is_selection_boundary_character(t_c.character) { Some(i + 1) } else { None }).unwrap_or(0)
    }
    pub fn word_end_index_of_first_character(&self) -> usize {
        self.columns.iter().enumerate().find_map(|(i, t_c)| if is_selection_boundary_character(t_c.character) { Some(i) } else { None }).unwrap_or_else(|| self.columns.len())
    }
}
