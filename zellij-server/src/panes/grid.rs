...
impl Grid {
    ...
    fn clear_egc_state_if_intersects(&mut self, y: usize, start_x: usize, end_x: usize) {
        if self
            .egc_state
            .as_ref()
            .is_some_and(|s| s.y == y && start_x < s.end_x && end_x > s.x)
        {
            self.egc_state = None;
        }
    }
    pub fn replace_characters_in_line_after_cursor(&mut self, replace_with: TerminalCharacter) {
        self.clear_egc_state_if_intersects(self.cursor.y, self.cursor.x, self.width);
    ...
    pub fn replace_characters_in_line_before_cursor(&mut self, replace_with: TerminalCharacter) {
        self.clear_egc_state_if_intersects(self.cursor.y, 0, self.cursor.x);
