...
//             let display_cols = self.get_columns();
            ...
            //    terminal_characters: Vec<TerminalCharacter>, (try not to allocate the vec)
            //    x: usize,
            //    y: usize,
            // }
            for character_chunk in self.grid.read_changes() {
                ...
                for t_character in terminal_characters {
                    // TODO: cut if it goes beyond pane bounds (accounting for wide characters!)
...
//             for (row_index, line) in buffer_lines.iter().enumerate() {
//                 let x = self.get_x();
//                 let y = self.get_y();
//                 vte_output.push_str(&format!("\u{1b}[{};{}H\u{1b}[m", y + row_index + 1, x + 1)); // goto row/col and reset styles
