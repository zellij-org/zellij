...
impl TiledPanes {
    ...
    pub fn render(
        ...
        let active_panes = if floating_panes_are_visible {
            ...
            // Include client entries that are either members of this
            // tab's `connected_clients` (regular focus) or carry the
            // explicit shadow-focus marker (e.g. a mobile client whose
            // plugin viewport is rendering a pane in this tab). The
            // marker distinguishes intentional shadow focus from
            // incidental `active_panes` entries — e.g. fake CLI client
            // ids that briefly land in `active_panes` during action
            // dispatch — which should not produce a focus indicator.
            self.active_panes
                .iter()
                ...
                for (client_id, active_pane_id) in active_panes {
                    if active_pane_id == pane_id {
                        if connected_clients.contains(&client_id) {
                            ...
                            // Shadow-focus client (e.g. mobile): the
                            // plugin picks its own replacement via
                            // PaneUpdate → sync_shadow_focus, so we
                            // just drop the entry. Silent to avoid
                            // CSI focus-tracking writes to the closing
                            // pane.
    ...
    // Shadow focus: record a client as focused on a pane *without* the
    // CSI focus-tracking writes or PTY-thread sync that the normal
    // focus path performs. Used by the mobile plugin so desktop
    // clients see the mobile client's focus marker on the pane the
    // mobile viewport is rendering.
    ...
    // (set by `insert_silent`/`set_shadow_focus`). Distinct from
    // "in `active_panes` but not in `connected_clients`" — the latter
    // also matches incidental entries like CLI fake client ids, which
    // should not be treated as shadow focus.
