...
impl TiledPanes {
    ...
    pub fn switch_active_pane_with(&mut self, pane_id: PaneId) {
        if let Some(active_pane_id) = self.first_active_pane_id() {
            ...
            resize_pty!(new_position, self.os_api, self.senders, self.character_cell_size).unwrap();
            ...
            resize_pty!(current_position, self.os_api, self.senders, self.character_cell_size).unwrap();
