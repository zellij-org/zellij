...
pub(crate) fn route_action(
    ...
    match action {
        ...
        Action::Write{key_with_modifier, bytes: raw_bytes, is_kitty_keyboard_protocol} => {
            senders
        ...
        Action::DumpScreen{file_path, include_scrollback} => {
            senders
                .send_to_screen(ScreenInstruction::DumpScreen(file_path, client_id, include_scrollback))
                .with_context(err_context)?;
        },
        Action::DumpLayout => {
        ...
        Action::NewPane{direction, pane_name, start_suppressed} => {
        ...
        Action::NewFloatingPane{command: run_command, pane_name: name, coordinates: floating_pane_coordinates} => {
        ...
        Action::NewInPlacePane{command: run_command, pane_name: name} => {
            let run_cmd = run_command
                .map(|cmd| TerminalAction::RunCommand(cmd.into()))
                .or_else(|| default_shell.clone());
            match pane_id {
        ...
        Action::NewStackedPane{command: run_command, pane_name: name} => {
            let run_cmd = run_command
                .map(|cmd| TerminalAction::RunCommand(cmd.into()))
                .or_else(|| default_shell.clone());
            match pane_id {
        ...
        Action::NewTiledPane{direction, command: run_command, pane_name: name} => {
            let run_cmd = run_command
                .map(|cmd| TerminalAction::RunCommand(cmd.into()))
                .or_else(|| default_shell.clone());
            let _ = senders.send_to_pty(PtyInstruction::SpawnTerminal(
        ...
        Action::NewTiledPluginPane{plugin: run_plugin, pane_name: name, skip_cache, cwd} => {
            senders
        ...
        Action::NewInPlacePluginPane{plugin: run_plugin, pane_name: name, skip_cache} => {
        ...
        Action::LaunchPlugin{plugin: run_plugin, should_float, should_open_in_place, skip_cache, cwd} => {
            senders
        ...
        Action::CloseTerminalPane{pane_id: terminal_pane_id} => {
            senders
        ...
        Action::ClosePluginPane{pane_id: plugin_pane_id} => {
            senders
                .send_to_screen(ScreenInstruction::ClosePane(
        ...
        Action::RenameTerminalPane{pane_id, name: name_bytes} => {
            senders
        ...
        Action::RenamePluginPane{pane_id, name: name_bytes} => {
            senders
                .send_to_screen(ScreenInstruction::RenamePane(
                    PaneId::Plugin(pane_id),
                    name_bytes,
        ...
        Action::RenameTab{tab_index: tab_position, name: name_bytes} => {
            senders
        ...
        Action::StackPanes{pane_ids: pane_ids_to_stack} => {
            senders
                ...
                >|
                 -> Result<bool> {
                    ...
                    match instruction {
                        ...
                        ClientToServerMsg::Action{action, terminal_id: maybe_pane_id, client_id: maybe_client_id} => {
                        ...
                        ClientToServerMsg::BackgroundColor{color: ref background_color_instruction} => {
                            send_to_screen_or_retry_queue!(
                                rlocked_sessions,
                        ...
                        ClientToServerMsg::ForegroundColor{color: ref foreground_color_instruction} => {
                            send_to_screen_or_retry_queue!(
                                rlocked_sessions,
                        ...
                        ClientToServerMsg::ColorRegisters{color_registers: ref color_registers} => {
                            send_to_screen_or_retry_queue!(
                                rlocked_sessions,
                                ScreenInstruction::TerminalColorRegisters(color_registers.iter().map(|c| (c.index, c.color.clone())).collect()),
                                instruction,
                                retry_queue
                            )
                            .with_context(err_context)?;
