...
impl Tab {
    ...
    pub fn handle_mouse_event(
        ...
        let context = self.gather_mouse_event_context(event, client_id)?;
    ...
    fn gather_mouse_event_context(
        ...
        let clicked_pane = pane_id_at_position
            .and_then(|id| self.gather_clicked_pane_details(id, &event.position, active_pane_id, event, client_id));
        ...
        Ok(MouseEventContext {
            ...
            active_pane_id,
            floating_visible,
            pane_being_resized: self.pane_being_resized_with_mouse.is_some(),
            selecting_with_mouse: self.selecting_with_mouse_in_pane.is_some(),
            pane_being_moved: self.floating_panes.pane_is_being_moved_with_mouse(),
            clicked_pane,
            pinned_selectable,
            pinned_unselectable,
        })
    ...
    fn gather_clicked_pane_details(
        ...
        let pane = self.get_pane_at(position, false).ok()??;
    ...
    fn execute_mouse_action(
        ...
        match action {
            ...
            MouseAction::StopResize { position } => {
                self.execute_stop_resize(position, client_id)
            }
            MouseAction::FocusPane { pane_id: _, position } => {
                self.execute_focus_pane(position, client_id)
            }
            ...
            MouseAction::EndSelection { position } => {
                self.execute_end_selection(position, client_id)
            }
            MouseAction::StartMovingFloatingPane { position } => {
                self.execute_move_floating_pane(position)
            }
            MouseAction::ContinueMovingFloatingPane { position } => {
                self.execute_move_floating_pane(position)
            }
            MouseAction::StopMovingFloatingPane { position } => {
                self.execute_stop_moving_floating_pane(position, client_id)
            }
            ...
            MouseAction::UpdateHover { pane_id } => {
                self.execute_update_hover(pane_id, client_id)
            }
            MouseAction::SendToTerminal { pane_id, event } => {
                self.execute_send_to_terminal(pane_id, event, client_id)
            }
    ...
    fn execute_move_floating_pane(&mut self, position: Position) -> Result<MouseEffect> {
        let search_selectable = false;
    ...
    fn determine_mouse_action(
        ...
        if event.ctrl && event.left && event.event_type == MouseEventType::Press {
            if let Some(details) = &ctx.clicked_pane {
                if details.on_frame {
                    ...
                    } else if let Some(edge) = details.edge {
        ...
        if event.left && event.event_type == MouseEventType::Press && !event.ctrl && !event.alt {
            if let Some(details) = &ctx.clicked_pane {
                let is_active = Some(details.pane_id) == ctx.active_pane_id;
                let is_pinned = ctx.pinned_selectable.map(|id| id == details.pane_id).unwrap_or(false);

                if details.on_frame {
                    if details.frame_intercepted {
                        return Ok(MouseAction::FrameIntercepted { pane_id: details.pane_id });
                    } else if ctx.floating_visible || is_pinned {
                ...
                if is_active {
                    ...
                    if !ctx.floating_visible {
                        ...
                        } else if ctx.pinned_unselectable.is_some() {
                            return Ok(MouseAction::NoAction);
                        }
                    }
        ...
        if event.right {
            if let Some(pane_id) = ctx.pane_id_at_position {
                if Some(pane_id) == ctx.active_pane_id {
