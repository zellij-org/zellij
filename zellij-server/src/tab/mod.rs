...
impl Tab {
    ...
    pub fn new_pane(
        ...
        if start_suppressed {
            ...
            if self.floating_panes.panes_are_visible() {
                self.add_floating_pane(new_pane, pid, floating_pane_coordinates, true, client_id)
            } else {
            ...
            match should_float {
                Some(true) => {
                    self.add_floating_pane(new_pane, pid, floating_pane_coordinates, false, client_id)
                },
        ...
        self.tiled_panes
            ...
            .or_else(|_| match self.suppressed_panes.remove(&pane_id) {
                Some(mut pane) => {
                    ...
                    if should_float {
                        self.show_floating_panes();
                        self.add_floating_pane(pane.1, pane_id, None, true, Some(client_id))
                    } else {
                        self.hide_floating_panes();
