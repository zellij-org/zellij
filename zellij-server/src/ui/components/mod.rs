...
use table::table;
use ribbon::{ribbon, emphasis_variants_for_ribbon, emphasis_variants_for_selected_ribbon};
use text::{text, Text, stringify_text, parse_text_params, parse_text};
...
impl <'a> UiComponentParser <'a> {
    ...
    pub fn parse(&mut self, bytes: Vec<u8>) -> Result<()> {
        ...
        if component_name == &"table" {
            ...
            let stringified_params = parse_text_params(params_iter).into_iter().next().with_context(|| format!("a ribbon must have text"))?;
            ...
            let encoded_nested_list = nested_list(nested_list_items, &self.style, component_coordinates);
            parse_vte_bytes!(self, encoded_nested_list);
            Ok(())
        } else if component_name == &"text" {
            let stringified_params = parse_text_params(params_iter).into_iter().next().with_context(|| format!("text must have, well, text..."))?;
