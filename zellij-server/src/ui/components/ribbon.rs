use super::{Coordinates, Text, is_too_wide};
...
pub fn ribbon(content: Text, style: &Style, arrow_fonts: bool, component_coordinates: Option<Coordinates>) -> Vec<u8> {
    let colors = style.colors;
    let (first_arrow_styles, text_style, last_arrow_styles) = if content.selected {
        (character_style(colors.black, colors.green), character_style(colors.black, colors.green), character_style(colors.green, colors.black))
    } else {
        (character_style(colors.black, colors.fg), character_style(colors.black, colors.fg), character_style(colors.fg, colors.black))
    };
    let (text, _text_width) = stringify_ribbon_text(&content, &component_coordinates, style, text_style);
    let mut stringified = component_coordinates.map(|c| c.to_string()).unwrap_or_else(|| String::new());
    let arrow = if arrow_fonts { ARROW_SEPARATOR } else { "" };
    stringified.push_str(&format!("{}{}{}{} {} {}{}{}", RESET_STYLES, first_arrow_styles, arrow, text_style, text, last_arrow_styles, arrow, RESET_STYLES));
...
fn stringify_ribbon_text(text: &Text, coordinates: &Option<Coordinates>, style: &Style, text_style: CharacterStyles) -> (String, usize) {
    ...
    for (i, character) in text.text.chars().enumerate() {
        ...
        if !text.indices.is_empty() {
            let character_with_styling = color_ribbon_index_character(character, i, &text, style, text_style);
...
fn color_ribbon_index_character(character: char, index: usize, text: &Text, style: &Style, base_style: CharacterStyles) -> String {
