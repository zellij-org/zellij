use super::{Coordinates, is_too_wide, emphasis_variants_for_ribbon, emphasis_variants_for_selected_ribbon, parse_selected, parse_indices};
use zellij_utils::{
...
pub fn text(content: Text, style: &Style, component_coordinates: Option<Coordinates>) -> Vec<u8> {
    ...
    let (text, _text_width) = stringify_text(&content, None, &component_coordinates, style, text_style);
    match component_coordinates {
        Some(component_coordinates) => {
            format!("{}{}{}", component_coordinates, text_style, text).as_bytes().to_vec()
        },
        None => {
...
pub fn stringify_text(text: &Text, left_padding: Option<usize>, coordinates: &Option<Coordinates>, style: &Style, text_style: CharacterStyles) -> (String, usize) {
    ...
    for (i, character) in text.text.chars().enumerate() {
        ...
        if is_too_wide(character_width, left_padding.unwrap_or(0) + text_width, &coordinates) {
            break;
        }
        text_width += character_width;
        if !text.indices.is_empty() {
            let character_with_styling = color_index_character(character, i, &text, style, text_style);
...
pub fn color_index_character(character: char, index: usize, text: &Text, style: &Style, base_text_style: CharacterStyles) -> String {
    let character_style = text.style_of_index(index, style)
