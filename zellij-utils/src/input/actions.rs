...
impl Action {
    ...
    pub fn actions_from_cli(
        ...
        match cli_action {
            ...
            CliAction::LaunchOrFocusPlugin { url, should_float } => {
                let run_plugin_location = RunPluginLocation::parse(url.as_str()).map_err(|e| {
                ...
                Ok(vec![Action::LaunchOrFocusPlugin(run_plugin, should_float)])
            }
        }
    }
}
