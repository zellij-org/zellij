...
impl Config {
    ...
    pub fn write_config_to_disk_if_it_does_not_exist(config: String, opts: &CliArgs) -> bool {
        ...
        match Config::config_file_path(opts) {
            Some(config_file_path) => {
                if config_file_exists(opts) {
                    ...
                    // TODO: CONTINUE HERE (13/08) - try to write the config, read it, and if it's the same
                    // as the one we wrote, return true
                    // (this should happen on startup)
        ...
        } else if config_file_exists(opts) {
            false
        } else {
