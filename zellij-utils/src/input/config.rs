...
impl Config {
            ...
            if let Some(config_file_path) = Config::config_file_path(&opts) {
                match std::fs::read_to_string(&config_file_path) {
                    Ok(current_config) => {
                        ...
                        let backed_up_config = std::fs::read_to_string(&backup_config_path).map_err(|e| {
                            log::error!("Failed to back up config file {}: {:?}", backup_config_path.display(), e);
                            Some(backup_config_path.clone())
                        })?;
    ...
    pub fn write_config_to_disk_if_it_does_not_exist(config: String, opts: &CliArgs) -> bool {
        fn config_file_exists(opts: &CliArgs) -> bool {
            Config::config_file_path(&opts).map(|path| path.exists()).unwrap_or(false)
        }
        log::info!("write_config_to_disk_if_it_does_not_exist 1");
        match Config::config_file_path(opts) {
            Some(config_file_path) => {
                log::info!("write_config_to_disk_if_it_does_not_exist 2");
                if config_file_exists(opts) {
                    log::info!("write_config_to_disk_if_it_does_not_exist 3");
                    false
                } else {
                    log::info!("write_config_to_disk_if_it_does_not_exist 4");
                    ...
                    match std::fs::read_to_string(&config_file_path) {
                        Ok(written_config) => {
                            log::info!("write_config_to_disk_if_it_does_not_exist 5");
            ...
            None => {
                log::info!("write_config_to_disk_if_it_does_not_exist 6");
                false
            }
        }
    }
