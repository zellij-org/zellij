...
impl Config {
    ...
    pub fn write_config_to_disk(config: String, opts: &CliArgs) -> Result<Config, String>{
        ...
        Config::from_kdl(&config, None)
            ...
            .and_then(|parsed_config| {
                ...
                let written_config = std::fs::read_to_string(config_file_path)
                    .map_err(|e| format!("Failed to read written config: {}", e))?;
                let parsed_written_config = Config::from_kdl(&written_config, None)
                    .map_err(|e| format!("Failed to parse written config: {}", e))?;
                if parsed_written_config == parsed_config {
                    Ok(parsed_config)
                }  else {
                    Err("Configuration corrupted when writing to disk".to_owned())
                }
            })
    }
}
