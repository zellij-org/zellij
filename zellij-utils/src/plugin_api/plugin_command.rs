pub use super::generated_api::api::{
    ...
    plugin_command::{
        ...
        WebRequestPayload, WriteCharsToPaneIdPayload, WriteToPaneIdPayload, ReloadPluginPayload, LoadNewPluginPayload
    },
    ...
    fn try_from(protobuf_plugin_command: ProtobufPluginCommand) -> Result<Self, &'static str> {
        match CommandName::from_i32(protobuf_plugin_command.name) {
            ...
            Some(CommandName::ReloadPlugin) => match protobuf_plugin_command.payload {
                Some(Payload::ReloadPluginPayload(
                    reload_plugin_payload,
                )) => Ok(PluginCommand::ReloadPlugin(
                    reload_plugin_payload.plugin_id,
                )),
                _ => Err("Mismatched payload for ReloadPlugin"),
            },
            Some(CommandName::LoadNewPlugin) => match protobuf_plugin_command.payload {
                Some(Payload::LoadNewPluginPayload(
                    load_new_plugin_payload,
                )) => Ok(PluginCommand::LoadNewPlugin {
                    url: load_new_plugin_payload.plugin_url,
                    config: load_new_plugin_payload.plugin_config
                        .into_iter()
                        .map(|e| (e.name, e.value))
                        .collect(),
                    load_in_background: load_new_plugin_payload.should_load_plugin_in_background,
    ...
    fn try_from(plugin_command: PluginCommand) -> Result<Self, &'static str> {
        match plugin_command {
            ...
            PluginCommand::ReloadPlugin(
                ...
                payload: Some(Payload::ReloadPluginPayload(
                    ReloadPluginPayload {
                        plugin_id,
                    },
                )),
            ...
            PluginCommand::LoadNewPlugin {
                ...
                payload: Some(Payload::LoadNewPluginPayload(
                    LoadNewPluginPayload {
                        plugin_url: url,
