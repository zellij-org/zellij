...
impl TryFrom<ProtobufPluginCommand> for PluginCommand {
   ...
   fn try_from(protobuf_plugin_command: ProtobufPluginCommand) -> Result<Self, &'static str> {
       match CommandName::from_i32(protobuf_plugin_command.name) {
           ...
           Some(CommandName::Resize) => {
               match protobuf_plugin_command.payload {
                   Some(Payload::ResizePayload(resize_payload)) => {
                       match resize_payload.resize {
                           Some(resize) => {
                               Ok(PluginCommand::Resize(resize.try_into()?))
                           },
           ...
           Some(CommandName::MoveFocus) => {
               match protobuf_plugin_command.payload {
                   Some(Payload::MoveFocusPayload(move_payload)) => {
                       match move_payload.direction {
                           Some(direction) => {
                               Ok(PluginCommand::MoveFocus(direction.try_into()?))
                           },
           ...
           Some(CommandName::MoveFocusOrTab) => {
               match protobuf_plugin_command.payload {
                   Some(Payload::MoveFocusOrTabPayload(move_payload)) => {
                       match move_payload.direction {
                           Some(direction) => {
                               Ok(PluginCommand::MoveFocusOrTab(direction.try_into()?))
                           },
           ...
           Some(CommandName::Write) => {
               match protobuf_plugin_command.payload {
                   Some(Payload::WritePayload(bytes)) => {
                       Ok(PluginCommand::Write(bytes))
                   },
                   _ => Err("Mismatched payload for Write")
               }
           }
           Some(CommandName::WriteChars) => {
               match protobuf_plugin_command.payload {
                   Some(Payload::WriteCharsPayload(chars)) => {
                       Ok(PluginCommand::WriteChars(chars))
                   },
           ...
           Some(CommandName::MovePaneWithDirection) => {
               match protobuf_plugin_command.payload {
           ...
           Some(CommandName::FocusOrCreateTab) => {
               match protobuf_plugin_command.payload {
           ...
           Some(CommandName::StartOrReloadPlugin) => {
               match protobuf_plugin_command.payload {
           ...
           Some(CommandName::ClosePluginPane) => {
               match protobuf_plugin_command.payload {
           ...
           Some(CommandName::FocusTerminalPane) => {
               match protobuf_plugin_command.payload {
           ...
           Some(CommandName::FocusPluginPane) => {
               match protobuf_plugin_command.payload {
           ...
           Some(CommandName::RenameTerminalPane) => {
               match protobuf_plugin_command.payload {
...
impl TryFrom<PluginCommand> for ProtobufPluginCommand {
   ...
   fn try_from(plugin_command: PluginCommand) -> Result<Self, &'static str> {
       match plugin_command {
           ...
           PluginCommand::ResizeWithDirection(resize) => {
               Ok(ProtobufPluginCommand {
           ...
           PluginCommand::MoveFocus(direction) => {
               Ok(ProtobufPluginCommand {
           ...
           PluginCommand::RenameTerminalPane(pane_id, new_name) => {
               Ok(ProtobufPluginCommand {
           ...
           PluginCommand::RenamePluginPane(pane_id, new_name) => {
               Ok(ProtobufPluginCommand {
           ...
           PluginCommand::ReportPanic(payload) => {
               Ok(ProtobufPluginCommand {
